...
export interface ActiveRider {
  ...
  lng: number;
  speed: number;
  pevType: PEVType | string;
  battery?: number;
  status?: string; // Added to sync with Radar statuses (e.g., "Charging", "Off-Roading")
...
export interface SavedRide {
  ...
  terrain?: string;
  powerMode?: string;
  totalWhConsumed?: number;
  totalAhConsumed?: number;
  ...
  rideScore?: string;
  tags?: string[];
  rideNote?: string; // Modern unified key
  notes?: string;    // Legacy fallback support
}
