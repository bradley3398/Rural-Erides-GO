...
import { 
  ...
  TrendingUp, SlidersHorizontal, ToggleLeft, ToggleRight, MessageSquareCode, Ruler, Grid, Database, Search, BrainCircuit, Wrench, Map, List, Download, Plus, Trash2, Edit3, Save, Check
} from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";

// --- NATIVE FIREBASE IMPORTS ---
import { collection, onSnapshot, doc, setDoc, increment, arrayUnion, serverTimestamp, addDoc, deleteDoc } from "firebase/firestore";
...
import WatchlistDrawer from "./Modals/WatchlistDrawer";
import FeedSettingsPanel from "./Modals/FeedSettingsPanel";
...
export default function YouTubeFeed(props: YouTubeFeedProps) {
  const [mounted, setMounted] = useState<boolean>(false);
  const [currentTheme, setCurrentTheme] = useState<string>(props.theme || 'lime');

  useEffect(() => {
    if (props.theme) setCurrentTheme(props.theme);
  }, [props.theme]);

  useEffect(() => {
    const handleThemeSync = () => {
      const savedTheme = localStorage.getItem("rural_theme") || localStorage.getItem("rt_theme");
  ...
  const [requestPevClass, setRequestPevClass] = useState<string>("Electric Scooter"); 
  const [isSubmittingRequest, setIsSubmittingRequest] = useState<boolean>(false); 

  // Watchlist Search & Notes States
  ...
  const [videoNotes, setVideoNotes] = useState<Record<string, string>>(() => {
    if (typeof window !== 'undefined') {
      try { return JSON.parse(localStorage.getItem("yt_video_notes") || "{}"); } catch (e) { return {}; }
    }
    return {};
  });
  ...
  // GLOBAL SYNC STATES (Omnibus)
  const [ytVolume, setYtVolume] = useState<number>(props.globalVolume ?? 100);
  const [ytVideoQuality, setYtVideoQuality] = useState<string>(() => {
    if (typeof window !== 'undefined') return localStorage.getItem("rural_stream_quality") || localStorage.getItem("yt_video_quality") || "large";
    return "large";
  });
  ...
  const getTheme = () => {
    const baseTheme = currentTheme || 'lime';
    const perf = props.performanceMode;
    const themes: Record<string, any> = {
      lime: { id: 'lime', primary: 'bg-[#39ff14]', text: 'text-[#39ff14]', border: 'border-[#39ff14]', hover: 'hover:bg-[#32e011]', glow: perf ? '' : 'shadow-[0_0_20px_rgba(57,255,20,0.3)]', bgSubtle: 'bg-[#39ff14]/10', borderSubtle: 'border-[#39ff14]/30', blur: perf ? 'bg-black/95' : 'bg-black/80 backdrop-blur-xl' },
      cyan: { id: 'cyan', primary: 'bg-cyan-500', text: 'text-cyan-400', border: 'border-cyan-500', hover: 'hover:bg-cyan-400', glow: perf ? '' : 'shadow-[0_0_20px_rgba(6,182,212,0.3)]', bgSubtle: 'bg-cyan-950/30', borderSubtle: 'border-cyan-900/50', blur: perf ? 'bg-black/95' : 'bg-black/80 backdrop-blur-xl' },
      emerald: { id: 'emerald', primary: 'bg-emerald-500', text: 'text-emerald-400', border: 'border-emerald-500', hover: 'hover:bg-emerald-400', glow: perf ? '' : 'shadow-[0_0_20px_rgba(16,185,129,0.3)]', bgSubtle: 'bg-emerald-950/30', borderSubtle: 'border-emerald-900/50', blur: perf ? 'bg-black/95' : 'bg-black/80 backdrop-blur-xl' },
      amber: { id: 'amber', primary: 'bg-amber-400', text: 'text-amber-400', border: 'border-amber-400', hover: 'hover:bg-amber-300', glow: perf ? '' : 'shadow-[0_0_20px_rgba(251,191,36,0.3)]', bgSubtle: 'bg-amber-950/30', borderSubtle: 'border-amber-900/50', blur: perf ? 'bg-black/95' : 'bg-black/80 backdrop-blur-xl' },
      rose: { id: 'rose', primary: 'bg-rose-500', text: 'text-rose-400', border: 'border-rose-500', hover: 'hover:bg-rose-400', glow: perf ? '' : 'shadow-[0_0_20px_rgba(244,63,94,0.3)]', bgSubtle: 'bg-rose-950/30', borderSubtle: 'border-rose-900/50', blur: perf ? 'bg-black/95' : 'bg-black/80 backdrop-blur-xl' },
      purple: { id: 'purple', primary: 'bg-purple-500', text: 'text-purple-400', border: 'border-purple-500', hover: 'hover:bg-purple-400', glow: perf ? '' : 'shadow-[0_0_20px_rgba(168,85,247,0.3)]', bgSubtle: 'bg-purple-950/30', borderSubtle: 'border-purple-900/50', blur: perf ? 'bg-black/95' : 'bg-black/80 backdrop-blur-xl' },
      void: { id: 'void', primary: 'bg-zinc-800', text: 'text-white', border: 'border-zinc-500', hover: 'hover:bg-zinc-700', glow: perf ? '' : 'shadow-[0_0_20px_rgba(255,255,255,0.1)]', bgSubtle: 'bg-zinc-900/50', borderSubtle: 'border-zinc-700/50', blur: perf ? 'bg-black/95' : 'bg-black/90 backdrop-blur-3xl' }
    };
  ...
  const scaleClass = fontScaleMap[props.uiScale as keyof typeof fontScaleMap] || "text-[14px]";
  ...
  useEffect(() => {
    if (props.globalVolume !== undefined) setYtVolume(props.globalVolume);
  }, [props.globalVolume]);
  ...
  const fetchTavilyWebGrounding = async (searchQueryText: string) => {
    ...
    try {
      ...
      console.warn("Tavily Grounded Search failed.", e);
    }
    return "";
  };
  ...
  const fetchLiveVideos = async () => {
    ...
    setErrorMsg(null);
    try {
      const cachedVideos = localStorage.getItem("yt_cache_videos");
      const cachedChannel = localStorage.getItem("yt_cache_channel");
      if (cachedVideos) setVideos(JSON.parse(cachedVideos));
      if (cachedChannel) setChannelData(JSON.parse(cachedChannel));
      ...
      if (statsData.items && statsData.items.length > 0) {
        ...
        const newChData = {
          ...
          desc: ch.snippet.description || "Broadcasting authentic PEV adventures, mechanical repairs, and raw outdoor trail scout runs.",
          avatar: ch.snippet.thumbnails?.high?.url || icon,
          subs: Number(statsData.items[0].statistics.subscriberCount || 0).toLocaleString(),
          views: Number(statsData.items[0].statistics.viewCount || 0).toLocaleString(),
          totalVideos: Number(statsData.items[0].statistics.videoCount || 0).toLocaleString(),
          customUrl: ch.snippet.customUrl || ""
        };
        setChannelData(newChData);
        localStorage.setItem("yt_cache_channel", JSON.stringify(newChData));
      }
      ...
      if (!searchRes.ok) throw new Error("Failed to fetch channel payload arrays.");
      ...
      if (searchData.items && searchData.items.length > 0) {
        ...
        if (videoIds) {
          ...
          const mappedVideos = searchData.items.map((i: any) => ({
            id: i.id.videoId, 
          ...
          localStorage.setItem("yt_cache_videos", JSON.stringify(mappedVideos));
        }
      }
    } catch (err: any) { 
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (data1.nextPageToken) {
        ...
        if (data2.items) combinedItems = [...combinedItems, ...data2.items];
      }
  ...
  const handleSendVideoRequest = async () => {
    ...
    try {
      await addDoc(collection(db, "board_posts"), {
        ...
        fleetSignature: userFleetConfig,
        content: `🎥 **VIDEO REQUEST:** ${requestTitle.trim()}`,
        category: "Video Requests",
        pevType: requestPevClass,
        template: "request",
        imageUrl: null,
        videoUrl: null,
        isHelpNeeded: false,
        ...
        volts: 0,
        comments: [],
        timestamp: new Date().toLocaleString(props.locale || "en", { hour: '2-digit', minute: '2-digit' }),
        timestamp_epoch: serverTimestamp(),
        isEdited: false
      });
      setRequestTitle("");
      setShowRequestModal(false);
      showToast("Video Request Broadcasted!");
    } catch (err) {
      showToast("Error transmitting video request.");
    } finally {
  ...
  const handleClearWatchlist = async () => {
    if (!window.confirm("Purge all videos from your saved watchlist vault?")) return;
    try {
      for (const item of savedWatchlist) {
        await deleteDoc(doc(db, "watchlists", item.id));
      }
      setSavedWatchlist([]);
      showToast("Watchlist Vault Cleared!");
    } catch (err) {
  ...
  const handleExportWatchlist = () => {
    if (savedWatchlist.length === 0) return showToast("Watchlist is empty.");
    const text = savedWatchlist.map(w => `- ${w.title}: https://youtu.be/${w.videoId}`).join("\n");
    navigator.clipboard.writeText(`RURAL FEED SAVED WATCHLIST:\n\n${text}`);
    showToast("Watchlist copied to clipboard!");
  };

  const handleSaveVideoNote = (videoId: string) => {
    if (!currentNoteInput.trim()) return;
    const updatedNotes = { ...videoNotes, [videoId]: currentNoteInput.trim() };
    setVideoNotes(updatedNotes);
    localStorage.setItem("yt_video_notes", JSON.stringify(updatedNotes));
    setCurrentNoteInput("");
    showToast("Note saved to video log!");
  };
  ...
  useEffect(() => {
    ...
    setUserFleetConfig(localStorage.getItem("universal_fleet_config") || "Standard PEV Fleet");
    setUserTerrainConfig(localStorage.getItem("universal_terrain_config") || "Mixed Road & Trails");
  ...
  useEffect(() => {
    if (mounted) {
      ...
      localStorage.setItem("yt_video_quality", ytVideoQuality);
    }
  }, [sortBy, aiPersonality, aiDetailLevel, hideDescriptionBox, autoTheaterOnSelect, autoLoadAiScan, showRawJsonData, aiCreativityLevel, aiThinkingLevel, aiUnitSystem, aiWebGrounding, aiOutputLength, gridDensity, hideLowEngagement, displayDiagnosticHUD, userFleetConfig, userTerrainConfig, ytVideoQuality, mounted]);
  ...
  useEffect(() => {
    if (activeVideo && scrollRef.current) {
      ...
      setVideoAiChat(null);
      setVideoQuestion("");
      setCurrentNoteInput(videoNotes[activeVideo.id] || "");
      if (autoLoadAiScan) {
        handleAIAction();
      }
    }
  ...
  const filteredWatchlist = savedWatchlist.filter(item => 
    item.title && item.title.toLowerCase().includes(watchlistSearch.toLowerCase())
  );
  ...
  return (
    <div className={`space-y-6 pb-24 font-sans text-zinc-100 min-h-screen transition-all duration-500 ${theaterMode ? "bg-black" : "bg-transparent"} ${scaleClass}`} ref={scrollRef}>
      
      {/* AMBIENT GLOW ORB */}
      {!props.performanceMode && !theaterMode && (
        <div className="absolute top-[5%] left-[20%] w-[600px] h-[600px] rounded-full opacity-10 blur-[150px] pointer-events-none -z-10" style={{ backgroundColor: t.hex }}></div>
      )}
      ...
      <AnimatePresence>
        {toastMessage && (
          <motion.div 
            ...
            className={`fixed top-20 left-1/2 -translate-x-1/2 z-[200] bg-black/80 backdrop-blur-2xl border ${t.border} px-5 py-3 rounded-xl flex items-center gap-3 ${t.glow}`}
          >
      ...
      {/* Navigation Header */}
      <nav className={`sticky top-0 ${t.blur} border-b border-white/10 z-50 px-4 py-3 flex flex-wrap items-center justify-between gap-3 shadow-xl transition-colors duration-500`}>
        ...
        <div className="flex items-center gap-2 flex-wrap min-w-0">
          <div className="relative flex items-center min-w-[140px] sm:min-w-[200px]">
             <input
               ...
               className="w-full bg-black/50 backdrop-blur-md border border-white/10 rounded-xl pl-3 pr-8 py-1.5 sm:py-2 text-[10px] text-white outline-none focus:border-white/30 transition-colors font-mono placeholder:text-zinc-400 shadow-inner"
             />
             <button type="button" onClick={() => handleSearch()} disabled={isFetching} className={`absolute right-2 ${t.text} hover:text-white cursor-pointer`}>
          ...
          <button 
            ...
            className="px-2.5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border bg-blue-950/40 border-blue-500/50 text-blue-400 hover:bg-blue-900/40 flex items-center gap-1 transition-all duration-300 active:scale-95 shrink-0 shadow-inner cursor-pointer"
            title="Request a Video"
          >
            <Video className="w-3.5 h-3.5 text-blue-400" /> Request Video
          </button>

          <button 
            type="button"
            onClick={() => setShowWatchlistDrawer(true)}
            className="px-2.5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border bg-white/5 border-white/10 text-zinc-300 hover:text-white flex items-center gap-1 transition-all duration-300 active:scale-95 shrink-0 shadow-inner cursor-pointer"
            title="Saved Watchlist"
          >
            <Bookmark className="w-3.5 h-3.5 text-amber-400" /> Watchlist ({savedWatchlist.length})
          </button>

          <button 
            type="button"
            onClick={() => setShowSettingsPanel(!showSettingsPanel)}
            className={`px-2.5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border flex items-center gap-1.5 transition-all duration-300 active:scale-95 shrink-0 cursor-pointer shadow-inner ${showSettingsPanel ? `${t.primary} text-black border-transparent ${t.glow}` : "bg-white/5 border-white/10 text-zinc-300 hover:text-white hover:bg-white/10"}`}
          >
            <Sliders className="w-3.5 h-3.5" /> Config
          </button>
          <button type="button" onClick={fetchLiveVideos} disabled={loading || activeVideo !== null} className="p-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl disabled:opacity-50 transition-all duration-300 active:scale-95 shadow-inner group shrink-0 cursor-pointer">
            <RefreshCw className={`w-4 h-4 text-zinc-300 group-hover:text-white ${loading ? "animate-spin" : ""}`} />
          </button>
        </div>
      </nav>

      {/* 🔥 MODULAR COMPONENTS INJECTED HERE 🔥 */}
      <VideoRequestModal {...{ showRequestModal, setShowRequestModal, requestTitle, setRequestTitle, requestPevClass, setRequestPevClass, isSubmittingRequest, handleSendVideoRequest }} />
      
      <WatchlistDrawer {...{ showWatchlistDrawer, setShowWatchlistDrawer, savedWatchlist, filteredWatchlist, watchlistSearch, setWatchlistSearch, handleExportWatchlist, handleClearWatchlist, setActiveVideo }} />
      
      <FeedSettingsPanel {...{
        showSettingsPanel, setShowSettingsPanel, t, userFleetConfig, setUserFleetConfig, userTerrainConfig, setUserTerrainConfig,
        aiPersonality, setAiPersonality, aiDetailLevel, setAiDetailLevel, aiThinkingLevel, setAiThinkingLevel, aiCreativityLevel, setAiCreativityLevel,
        hideDescriptionBox, setHideDescriptionBox, autoTheaterOnSelect, setAutoTheaterOnSelect, aiUnitSystem, setAiUnitSystem, gridDensity, setGridDensity,
        aiWebGrounding, setAiWebGrounding, aiOutputLength, setAiOutputLength, autoLoadAiScan, setAutoLoadAiScan, displayDiagnosticHUD, setDisplayDiagnosticHUD, showRawJsonData, setShowRawJsonData
      }} />
      ...
      <div className={`p-4 space-y-6 mx-auto transition-all duration-500 ${theaterMode ? "max-w-[1600px]" : "max-w-7xl"}`}>
        ...
        {activeVideo ? (
          <motion.div 
            ...
            className={`bg-black/60 backdrop-blur-3xl border rounded-3xl overflow-hidden border-white/10 shadow-[0_8px_32px_0_rgba(0,0,0,0.36)]`}
          >
            {/* Viewport Asset Frame */}
            <div className={`relative w-full bg-black border-b border-white/10 transition-all duration-500 ${theaterMode ? "aspect-[21/9]" : "aspect-video"}`}>
              <iframe 
                src={`https://www.youtube.com/embed/${activeVideo.id}?autoplay=1&vq=${ytVideoQuality}`} 
                title={activeVideo.title} 
                frameBorder="0" 
              ...
              <div className="absolute top-4 right-4 flex flex-wrap items-center gap-2 z-10">
                <button type="button" onClick={() => setTheaterMode(!theaterMode)} className="p-2 bg-black/80 backdrop-blur-md rounded-xl text-zinc-300 hover:text-white border border-white/10 shadow-xl transition-all duration-300 active:scale-95 cursor-pointer"><Maximize2 className="w-5 h-5" /></button>
                <button type="button" onClick={() => { setActiveVideo(null); setAiStatus(null); setVideoAiChat(null); setTheaterMode(false); }} className="p-2 bg-black/80 backdrop-blur-md rounded-xl text-zinc-300 hover:text-red-400 border border-white/10 shadow-xl transition-all duration-300 active:scale-95 cursor-pointer"><X className="w-5 h-5" /></button>
              </div>
            </div>

            {/* Live Diagnostics Telemetry Sub-Ribbon */}
            {displayDiagnosticHUD && (
              <div className="bg-black/60 backdrop-blur-md border-b border-white/10 px-5 py-3 grid grid-cols-2 sm:grid-cols-4 gap-4 text-center font-mono text-[10px] shadow-inner">
                 <div className="border-r border-white/10 py-1"><span className="text-zinc-500 uppercase block text-[8px] font-black">Metadata Stream Node</span><span className="text-white font-bold truncate block px-1">{activeVideo.id}</span></div>
                 <div className="border-r border-white/10 py-1"><span className="text-zinc-500 uppercase block text-[8px] font-black">Extracted Video Runtime</span><span className={`${t.text} font-bold`}>{activeVideo.duration}</span></div>
                 <div className="border-r border-white/10 py-1"><span className="text-zinc-500 uppercase block text-[8px] font-black">Data Grounding Target</span><span className="text-emerald-400 font-bold uppercase">Backend API Tags</span></div>
                 <div className="py-1"><span className="text-zinc-500 uppercase block text-[8px] font-black">Google Web Search</span><span className={aiWebGrounding ? "text-cyan-400 font-bold" : "text-zinc-500 font-bold"}>{aiWebGrounding ? "ACTIVE" : "DISABLED"}</span></div>
              </div>
            )}

            <div className={`p-5 space-y-6 transition-all duration-500 ${theaterMode ? "md:p-8 max-w-5xl mx-auto" : "md:p-6"}`}>
              <div className="flex flex-wrap items-center gap-3">
                <span className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest border shadow-inner ${activeVideo.category?.color || "text-zinc-100 bg-white/5 border-white/10"}`}>
                  {activeVideo.category?.label || "Stream"}
                </span>
                <span className="px-3 py-1.5 bg-white/5 border border-white/10 backdrop-blur-md text-zinc-200 rounded-lg text-[10px] font-bold flex items-center gap-2 shadow-inner"><Eye className={`w-3.5 h-3.5 ${t.text}`} /> {Number(activeVideo.views).toLocaleString()} VIEWS</span>
                <span className="px-3 py-1.5 bg-white/5 border border-white/10 backdrop-blur-md text-zinc-200 rounded-lg text-[10px] font-bold flex items-center gap-2 shadow-inner"><ThumbsUp className={`w-3.5 h-3.5 ${t.text}`} /> {Number(activeVideo.ytLikes).toLocaleString()} LIKES</span>
                <span className="px-3 py-1.5 bg-white/5 border border-white/10 backdrop-blur-md text-zinc-400 rounded-lg text-[10px] font-bold flex items-center gap-2 shadow-inner ml-auto"><Calendar className="w-3.5 h-3.5" /> {new Date(activeVideo.publishedAt).toLocaleDateString(props.locale || "en")}</span>
              </div>
              ...
              {activeVideo.rawTagsArray && activeVideo.rawTagsArray.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {activeVideo.rawTagsArray.slice(0, 8).map((tag: string, idx: number) => (
                    <span key={idx} className="px-2 py-1 bg-black/40 border border-white/10 rounded text-[9px] font-mono text-zinc-300 uppercase shadow-inner">#{tag}</span>
                  ))}
                  {activeVideo.rawTagsArray.length > 8 && <span className="px-2 py-1 bg-black/40 border border-white/10 rounded text-[9px] font-mono text-zinc-400 uppercase shadow-inner">+{activeVideo.rawTagsArray.length - 8} MORE</span>}
                </div>
              )}

              {/* Functional Payload Triggers */}
              <div className="flex flex-wrap gap-2.5 sm:gap-3">
                <button 
                  type="button"
                  onClick={handleAIAction} disabled={aiLoading}
                  className={`flex-1 min-w-[200px] py-3.5 sm:py-4 px-4 rounded-xl border-2 flex items-center justify-center gap-2 text-xs font-black uppercase tracking-widest transition-all duration-300 active:scale-95 cursor-pointer shadow-lg ${aiLoading || aiStatus ? `${t.border} ${t.text} ${t.bgSubtle} ${t.glow}` : `bg-white/5 border-white/10 backdrop-blur-md text-zinc-200 hover:text-white hover:border-white/20`}`}
                >
                ...
                <button 
                  ...
                  className={`flex-1 min-w-[140px] py-3.5 sm:py-4 rounded-xl flex items-center justify-center gap-2 text-xs font-black uppercase tracking-widest transition-all duration-300 active:scale-95 cursor-pointer shadow-inner border ${
                    votedVolts.includes(activeVideo.id) 
                      ? "bg-amber-950/40 border-amber-500/50 text-amber-400 cursor-not-allowed" 
                      : "bg-white/5 border-white/10 backdrop-blur-md hover:border-amber-500/50 text-zinc-300 hover:text-amber-400"
                  }`}
                >
                ...
                <button type="button" onClick={(e) => { e.stopPropagation(); navigator.clipboard.writeText(`https://youtu.be/${activeVideo.id}`); showToast("Video Link Copied!"); }} className={`p-3.5 sm:p-4 rounded-xl bg-white/5 border border-white/10 backdrop-blur-md text-zinc-300 hover:text-white hover:border-white/20 shrink-0 shadow-inner transition-all duration-300 active:scale-95 cursor-pointer`}>
                  <Share2 className="w-4.5 h-4.5" />
                </button>
              </div>

              {/* Conversational AI Output Array */}
              <div className={`bg-black/60 backdrop-blur-3xl border rounded-2xl p-6 shadow-inner relative overflow-hidden transition-all ${aiStatus ? t.borderSubtle : "border-white/10"}`}>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 mb-4 border-b border-white/10 pb-3">
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-lg ${aiStatus ? t.bgSubtle : "bg-white/5"}`}><Cpu className={`w-4 h-4 ${aiStatus ? t.text : "text-zinc-400"}`} /></div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {aiStatus && (
                      <button type="button" onClick={exportAiSummary} className={`px-3 py-1 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-[9px] font-mono font-bold ${t.text} uppercase flex items-center gap-1.5 transition-all duration-300 active:scale-95 cursor-pointer shadow-inner`}>
              ...
              {/* --- 🔥 NEW FEATURE: PERSONAL VIDEO NOTES & TIMESTAMPS 🔥 --- */}
              <div className="bg-black/40 backdrop-blur-md border border-white/10 rounded-2xl p-5 shadow-inner space-y-3">
                <div className="flex items-center justify-between border-b border-white/10 pb-2.5">
                   <span className="text-xs font-black uppercase tracking-widest text-amber-400 flex items-center gap-2"><Edit3 className="w-4 h-4"/> Personal Video Notes &amp; Timestamps</span>
                   <span className="text-[9px] font-mono text-zinc-500">Saved Locally</span>
                </div>
                <div className="flex gap-2">
                  <input 
                    ...
                    onKeyDown={(e) => e.key === 'Enter' && handleSaveVideoNote(activeVideo.id)}
                    placeholder="Add a timestamp note (e.g. 04:22 - Controller wiring fix)..."
                    className="flex-1 bg-black/50 border border-white/10 rounded-xl px-3 py-2.5 text-xs text-white outline-none focus:border-amber-500 font-bold shadow-inner"
                  />
                  <button type="button" onClick={() => handleSaveVideoNote(activeVideo.id)} className="bg-amber-500 hover:bg-amber-400 text-black px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest cursor-pointer active:scale-95 transition-all shadow-md shrink-0">
                    <Save className="w-3.5 h-3.5"/> Save Note
                  </button>
                </div>
                {videoNotes[activeVideo.id] && (
                  <div className="bg-amber-950/20 border border-amber-500/30 p-3 rounded-xl text-xs text-amber-200 font-mono">
                    <strong>Saved Note:</strong> {videoNotes[activeVideo.id]}
                  </div>
                )}
              </div>

              {/* DIRECT VIDEO INFO CONTEXT INTERROGATION CHAT MODULE */}
              <div className="bg-black/40 backdrop-blur-md border border-white/10 rounded-2xl p-5 shadow-inner space-y-4">
                ...
                {videoAiChat && (
                  <div className="p-4 rounded-xl bg-black/60 border border-white/10 font-sans text-zinc-100 text-xs sm:text-sm leading-relaxed font-bold transition-all shadow-inner">
                     {videoAiChat}
                  </div>
                )}
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  <input 
                    ...
                    className="flex-1 bg-black/50 backdrop-blur-md border border-white/10 rounded-xl px-4 py-3 text-xs text-white placeholder:text-zinc-500 outline-none focus:border-white/30 transition-colors shadow-inner font-bold"
                  />
                  <button 
                    type="button" 
                    onClick={handleInterrogateVideo}
                    disabled={videoChatLoading || !videoQuestion.trim()}
                    className={`px-5 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-1.5 transition-all duration-300 active:scale-95 cursor-pointer ${t.primary} text-black disabled:opacity-35 shadow-lg w-full sm:w-auto shrink-0`}
                  >
              ...
              {!hideDescriptionBox && (
                <div className="bg-white/5 backdrop-blur-md border border-white/10 p-5 rounded-2xl text-xs font-bold text-zinc-300 font-sans leading-relaxed space-y-2 shadow-inner">
                  <span className="text-[9px] font-black text-zinc-400 uppercase tracking-widest block font-mono">Raw Native Video Metadata Logs</span>
                  <p className="line-clamp-6 whitespace-pre-wrap">{activeVideo.description}</p>
                </div>
              )}

              {/* Discussion Channel Comms */}
              <div className="bg-black/60 backdrop-blur-2xl border border-white/10 rounded-2xl overflow-hidden shadow-2xl flex flex-col">
                ...
                <div className="p-5 space-y-4 max-h-[350px] overflow-y-auto custom-scrollbar bg-black/40">
                  {(!activeEng?.comments || activeEng.comments.length === 0) ? (
                    <div className="border-2 border-dashed border-white/10 rounded-xl p-10 flex flex-col items-center justify-center bg-white/5">
                      <MessageCircle className="w-8 h-8 text-zinc-500 mb-3" />
                    ...
                    activeEng.comments.map((reply: any) => (
                      <div key={reply.id} className="bg-white/5 border border-white/10 p-4 rounded-xl text-xs shadow-inner backdrop-blur-md">
                        <div className="flex justify-between items-center mb-2">
                          <div className="flex items-center gap-2">
                            <div className={`w-5 h-5 ${t.primary} text-black font-black flex items-center justify-center rounded text-[9px] uppercase shadow-sm`}>{reply.author.substring(0, 2)}</div>
                ...
                <div className="p-4 bg-black/80 border-t border-white/10 flex gap-3">
                  <input 
                    ...
                    className="flex-1 bg-black/50 backdrop-blur-md border border-white/10 rounded-xl px-4 py-3.5 text-xs text-white focus:outline-none focus:border-white/30 transition-colors shadow-inner font-bold"
                  />
                  <button type="button" onClick={() => handlePostComment(activeVideo.id)} disabled={!commentText.trim()} className={`${t.primary} text-black px-6 rounded-xl font-black uppercase tracking-widest text-[10px] flex items-center gap-2 transition-all duration-300 active:scale-95 disabled:opacity-35 shrink-0 shadow-md cursor-pointer`}>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            <div className="bg-black/40 backdrop-blur-2xl border border-white/10 p-6 md:p-8 rounded-3xl shadow-[0_8px_32px_0_rgba(0,0,0,0.36)] flex flex-col md:flex-row justify-between items-center gap-8 relative overflow-hidden mb-6">
              {!props.performanceMode && <div className={`absolute top-0 right-0 w-96 h-96 ${t.primary} opacity-5 rounded-full blur-[100px] -mr-20 -mt-20 pointer-events-none`}></div>}
              ...
              <div className="relative z-10 flex flex-col md:flex-row items-center md:items-start gap-6 w-full md:w-auto text-center md:text-left">
                {channelData.avatar ? (
                  <div className={`w-24 h-24 md:w-32 md:h-32 rounded-full bg-black border-4 ${t.border} ${t.glow} shrink-0 overflow-hidden p-1.5 relative shadow-xl`}>
                  ...
                  <div className="w-24 h-24 rounded-full bg-white/5 border border-white/10 animate-pulse shrink-0"></div>
                )}
                <div className="max-w-xl flex flex-col justify-center">
                  ...
                  <h2 className="text-white font-black text-3xl md:text-4xl tracking-widest mb-3 uppercase drop-shadow-md">
                    {channelData.title}
                  </h2>
                  <p className="text-xs md:text-sm text-zinc-300 font-bold leading-relaxed italic border-l-2 border-white/20 pl-4 mb-4">
                    "{channelData.desc}"
                  </p>
                  ...
                  <a 
                    ...
                    className={`w-full sm:w-fit px-5 py-3 rounded-xl bg-red-600 hover:bg-red-500 text-white font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-2 shadow-lg hover:shadow-red-600/20 transition-all duration-300 active:scale-95 mt-2 cursor-pointer`}
                  >
              ...
              <div className="grid grid-cols-3 md:grid-cols-1 gap-3 relative z-10 w-full md:w-48 shrink-0">
                <div className="flex flex-col items-center justify-center text-center gap-1.5 bg-white/5 backdrop-blur-md border border-white/10 p-4 rounded-2xl shadow-inner">
            ...
            {errorMsg && (
              <div className="bg-red-950/45 border border-red-500/40 text-red-200 text-sm rounded-xl p-4 flex gap-3 items-center shadow-lg mb-6 font-mono uppercase backdrop-blur-md">
                <AlertTriangle className="w-6 h-6 text-red-400 shrink-0" /> <p>{errorMsg}</p>
              </div>
            )}

            {/* ACTION INTERFACE ENGINE: CATEGORIES & SORT pills */}
            <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 bg-black/40 backdrop-blur-2xl border border-white/10 p-3 rounded-2xl shadow-[0_8px_32px_0_rgba(0,0,0,0.36)] mb-6">
              <div className="flex flex-wrap gap-2 w-full xl:w-auto">
                {["All", "PEV Review", "Garage / Build", "Trail Scout", "General Ride"].map((tab) => (
                  <button
                    key={tab} type="button" onClick={() => setActiveFilter(tab)}
                    className={`flex-1 xl:flex-none px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all duration-300 active:scale-95 border cursor-pointer ${activeFilter === tab ? `${t.bgSubtle} ${t.text} ${t.border} shadow-inner` : "bg-white/5 text-zinc-300 border-white/10 hover:text-white hover:bg-white/10"}`}
                  >
                    {tab}
                  </button>
                ))}
              ...
              <button 
                ...
                className={`w-full xl:w-auto flex justify-center items-center gap-2 px-5 py-2.5 bg-white/5 border border-white/10 hover:border-white/20 rounded-xl text-[10px] font-black text-zinc-200 hover:text-white transition-all duration-300 active:scale-95 shadow-inner uppercase tracking-widest cursor-pointer`}
              >
            ...
            {loading ? (
              ...
              <div className="bg-black/40 backdrop-blur-2xl border border-white/10 rounded-3xl p-16 text-center shadow-xl w-full">
                ...
                {processed.map((v) => {
                  ...
                  return (
                    <div 
                      ...
                      className={`group cursor-pointer bg-black/40 backdrop-blur-2xl rounded-3xl border border-white/10 hover:border-white/30 overflow-hidden flex flex-col transition-all duration-300 shadow-[0_8px_32px_0_rgba(0,0,0,0.36)] hover:${t.glow} hover:-translate-y-0.5`}
                    >
                      <div className="relative w-full aspect-video bg-black overflow-hidden border-b border-white/10">
                        ...
                        <div className={`absolute top-3 left-3 px-2.5 py-1 rounded-lg bg-black/80 backdrop-blur-md text-[9px] font-black uppercase tracking-widest border shadow-lg ${v.category?.color || 'text-zinc-100 border-zinc-600'}`}>
                          {v.category?.label || 'Stream'}
                        </div>
                        <div className="absolute top-3 right-3 flex flex-col gap-1.5 items-end">
                          <div className="px-2 py-1 rounded-lg bg-black/80 backdrop-blur-md text-[10px] font-mono font-bold text-white flex items-center gap-1.5 border border-white/10 shadow-lg">
                            <Eye className={`w-3.5 h-3.5 ${t.text}`} /> {Number(v.views).toLocaleString()}
                          </div>
                          <div className="px-2 py-1 rounded-lg bg-black/80 backdrop-blur-md text-[9px] font-mono font-bold text-zinc-300 flex items-center gap-1.5 border border-white/10 shadow-lg">
                        ...
                        <div className="flex items-center justify-between border-t border-white/10 pt-4">
                          <div className="flex gap-2">
                            ...
                            <button 
                              ...
                              className={`flex items-center gap-1.5 px-3 py-1.5 bg-white/5 border rounded-lg text-xs font-bold transition-all duration-300 active:scale-95 shadow-inner cursor-pointer ${
                                votedVolts.includes(v.id) 
                                  ? "border-amber-500/50 text-amber-400 cursor-not-allowed bg-amber-950/40" 
                                  : "border-white/10 hover:border-amber-500/50 text-zinc-300 hover:text-amber-400"
                              }`}
                            >
                              <Flame className={`w-3.5 h-3.5 ${votedVolts.includes(v.id) ? "text-amber-500 fill-amber-500" : ""}`} /> 
                              {eng?.volts || 0}
                            </button>
                            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white/5 border border-white/10 rounded-lg text-xs font-bold text-zinc-300 shadow-inner">
                              <MessageCircle className="w-3.5 h-3.5 text-zinc-400" /> {eng.comments?.length || 0}
                            </div>
                          </div>
                          
                          <div className="flex items-center gap-2">
                            <button type="button" onClick={(e) => handleBookmark(e, v)} className="p-2 bg-white/5 rounded-lg text-zinc-400 hover:text-white transition-all duration-300 active:scale-95 border border-white/10 shadow-inner cursor-pointer" title="Save to Watchlist">
                              <Bookmark className="w-4 h-4" />
                            </button>
                            <button type="button" onClick={(e) => { e.stopPropagation(); navigator.clipboard.writeText(`https://youtu.be/${v.id}`); showToast("Video Link Copied!"); }} className="p-2 bg-white/5 rounded-lg text-zinc-400 hover:text-white transition-all duration-300 active:scale-95 border border-white/10 shadow-inner cursor-pointer" title="Share Link">
            ...
            {showRawJsonData && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="bg-black/80 backdrop-blur-md border border-white/10 rounded-2xl p-4 mt-6 shadow-inner font-mono text-[9px] text-zinc-400 overflow-x-auto">
